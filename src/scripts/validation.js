/**
 * VALIDATION.JS - VERSION COMPLÈTEMENT CORRIGÉE
 * 
 ...
 * - DoubleNumber: 8 doubles [11,22,33,44,55,66,77,88] + TOUTES LES FORMULES (Directe, Turbo2/3/4, etc.)
 * - Annagrammesimple: 37 binômes automatiques + TOUTES LES FORMULES (Directe, Turbo2/3/4, etc.)
 * ⚠️ Ces 2 types utilisent des listes fixes mais appliquent la logique de la formule choisie !
 *
 * EXEMPLES :
 * - DoubleNumber + Directe : ≥ 2 doubles trouvés dans tous les tirés
 * - DoubleNumber + Turbo2 : ≥ 2 doubles trouvés dans les 2 premiers tirés
 * - Anagramme + Directe : ≥ 1 binôme complet dans tous les tirés  
 * - Anagramme + Turbo3 : ≥ 1 binôme complet dans les 3 premiers tirés
 */
...
function validateSingleTicket(ticket, winningNumbers, winningNumbers2, game) {
  ...
  try {
    if (Array.isArray(numerosJoues)) {
      ...
      // Peut être un JSON string ou une chaîne séparée par des virgules
      if (numerosJoues.startsWith('[')) {
        playedNumbers = JSON.parse(numerosJoues).map(num => parseInt(num));
      } else {
        playedNumbers = numerosJoues.split(',').map(num => parseInt(num.trim()));
      }
    } else {
      playedNumbers = JSON.parse(numerosJoues).map(num => parseInt(num));
    }
    ...
    console.error(`❌ numerosJoues reçu:`, numerosJoues);
    return false;
  }
...
function validateByFormula(formule, playedNumbers, winningNumbers, typeJeu = null) {
  ...
  switch (formuleClean) {
    ...
    case 'Turbo3':
      if (playedNumbers.length === 2) {
        console.log(`    📝 Fallback: 2 numéros → Two Sure Turbo3`);
        return validateTurbo(playedNumbers, winningNumbers, 3);
      } else {
        console.log(`    📝 Fallback: ${playedNumbers.length} numéros → Permutation Turbo3`);
        return validateTurboPermutation(playedNumbers, winningNumbers, 3);
      }
    case 'Turbo4':
      if (playedNumbers.length === 2) {
        console.log(`    📝 Fallback: 2 numéros → Two Sure Turbo4`);
        return validateTurbo(playedNumbers, winningNumbers, 4);
      } else {
        console.log(`    📝 Fallback: ${playedNumbers.length} numéros → Permutation Turbo4`);
        return validateTurboPermutation(playedNumbers, winningNumbers, 4);
      }
    ...
    // === Autres fallbacks par formule pour Anagramme et DoubleNumber ===
    case 'Turbo2':
      // Auto-détection : pourrait être Two Sure, Permutation, Anagramme ou DoubleNumber
      if (playedNumbers.length === 2) {
        console.log(`    📝 Fallback: 2 numéros → Two Sure Turbo2`);
        return validateTurbo(playedNumbers, winningNumbers, 2);
      } else if (playedNumbers.length === 8) {
        console.log(`    📝 Fallback: 8 numéros → Double Number Turbo2`);
        return validateDoubleNumberFormula(winningNumbers, 'Turbo2');
      } else if (playedNumbers.length === 74) {
        console.log(`    📝 Fallback: 74 numéros → Anagramme Turbo2`);
        return validateAnagrammeFormula(winningNumbers, 'Turbo2');
      } else {
        console.log(`    📝 Fallback: ${playedNumbers.length} numéros → Permutation Turbo2`);
        return validateTurboPermutation(playedNumbers, winningNumbers, 2);
    ...
    case 'Turbo3':
      if (playedNumbers.length === 2) {
        ...
        console.log(`    📝 Fallback: 8 numéros → Double Number Turbo3`);
        return validateDoubleNumberFormula(winningNumbers, 'Turbo3');
      } else if (playedNumbers.length === 74) {
        console.log(`    📝 Fallback: 74 numéros → Anagramme Turbo3`);
        return validateAnagrammeFormula(winningNumbers, 'Turbo3');
      } else {
        console.log(`    📝 Fallback: ${playedNumbers.length} numéros → Permutation Turbo3`);
        return validateTurboPermutation(playedNumbers, winningNumbers, 3);
    ...
    case 'Turbo4':
      if (playedNumbers.length === 2) {
        ...
        console.log(`    📝 Fallback: 8 numéros → Double Number Turbo4`);
        return validateDoubleNumberFormula(winningNumbers, 'Turbo4');
      } else if (playedNumbers.length === 74) {
        console.log(`    📝 Fallback: 74 numéros → Anagramme Turbo4`);
        return validateAnagrammeFormula(winningNumbers, 'Turbo4');
      } else {
        console.log(`    📝 Fallback: ${playedNumbers.length} numéros → Permutation Turbo4`);
        return validateTurboPermutation(playedNumbers, winningNumbers, 4);
...
function validateFirstBKDirecte(playedNumbers, winningNumbers) {
  ...
  console.log(`    ${isFound ? '✅' : '❌'} Numéro ${playedNumber} ${isFound ? 'trouvé' : 'NON trouvé'} parmi les tirés`);
...
function validatePosition(playedNumbers, winningNumbers, position) {
  ...
  console.log(`    ${isValid ? '✅' : '❌'} Numéro à position ${position}: ${targetNumber} ${isValid ? 'trouvé' : 'NON trouvé'} dans joués`);
...
function validateTurbo(playedNumbers, winningNumbers, topCount) {
  ...
  console.log(`    ${allFound ? '✅' : '❌'} ${foundNumbers.length}/${playedNumbers.length} numéros trouvés dans les ${topCount} premiers`);
  console.log(`    📊 Numéros trouvés: [${foundNumbers.join(', ')}]`);
...
function validateTurboPermutation(playedNumbers, winningNumbers, topCount) {
  ...
  console.log(`    ${isValid ? '✅' : '❌'} ${foundNumbers.length} correspondances dans les ${topCount} premiers (min requis: 2)`);
  console.log(`    📊 Numéros trouvés: [${foundNumbers.join(', ')}]`);
...
function validateAnagrammeFormula(winningNumbers, formule) {
  ...
  switch (formule) {
    case 'Directe':
      ...
      gagnants = anagrammes.filter(([num1, num2]) => 
    ...
    case 'Turbo2':
      ...
      console.log(`    📊 2 premiers tirés: [${top2.join(', ')}]`);
      gagnants = anagrammes.filter(([num1, num2]) => 
        top2.includes(num1) && top2.includes(num2)
      );
      break;
    ...
    case 'Turbo3':
      ...
      console.log(`    📊 3 premiers tirés: [${top3.join(', ')}]`);
      gagnants = anagrammes.filter(([num1, num2]) => 
        top3.includes(num1) && top3.includes(num2)
      );
      break;
    ...
    case 'Turbo4':
      ...
      console.log(`    📊 4 premiers tirés: [${top4.join(', ')}]`);
      gagnants = anagrammes.filter(([num1, num2]) => 
        top4.includes(num1) && top4.includes(num2)
      );
      break;
  ...
  if (isValid) {
    ...
    gagnants.slice(0, 3).forEach(([num1, num2]) => { // Affiche max 3 pour éviter le spam
